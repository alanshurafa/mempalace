...
- **`MEMPAL_PYTHON`** — Optional env var. Python interpreter with mempalace + chromadb installed. Auto-detects: `MEMPAL_PYTHON` env var (`MEMPALACE_PYTHON` accepted as a back-compat alias) → repo `.venv` → system `python3`. Set this if your venv is in a non-standard location.
